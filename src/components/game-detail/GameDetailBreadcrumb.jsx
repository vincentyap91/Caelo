            ...
            {items.map((item, index) => {
                ...
                return (
                    <React.Fragment key={`${item.label}-${index}`}>
                        ...
                        {isLast ? (
                            <span className="inline-flex items-center rounded-full bg-[var(--color-secondary)] px-3 py-1 text-xs font-bold text-[var(--color-tertiery)] shadow-sm md:text-sm">
                                {item.label}
                            </span>
                            ...
                            <button
                                ...
                                className="font-medium text-[var(--color-text-muted)] transition hover:text-[var(--color-secondary)]"
                            >
                                {item.label}
                            </button>
                        ) : (

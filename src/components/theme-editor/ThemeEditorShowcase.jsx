...
function PreviewCard({ title, children }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            padding: '3px 8px', borderRadius: 8, border: '1px solid var(--color-border-subtle)',
            background: dark ? '#1e293b' : 'var(--color-tertiery)',
      ...
      <div style={{
        padding: 14,
        background: dark ? '#0f172a' : 'var(--color-tertiery)',
        color: dark ? '#e2e8f0' : 'var(--color-text-secondary)',
      ...
      <div style={{
        ...
        {['Dashboard', 'Deposit', 'Withdrawal', 'History'].map((item, i) => (
          <div key={item} style={{
            ...
            color: i === 0 ? 'var(--color-accent)' : 'var(--color-tertiery)',
            cursor: 'pointer',
      ...
      <div className="dark-nav-shell" style={{ borderRadius: 10, padding: 10 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
          {['Pragmatic Play', 'PG Soft', 'Jili', 'Spadegaming'].map(p => (
            <div key={p} className="dark-nav-tile" style={{ padding: '6px 8px', borderRadius: 7, fontSize: 11, fontWeight: 600, color: 'var(--color-tertiery)' }}>{p}</div>
          ))}
        </div>
      </div>
      {/* SegmentedTabs */}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <tbody>
            {[
              ...
              <tr key={i} style={{ borderBottom: '1px solid var(--color-border-subtle)' }}>
                ...
                <td style={{ padding: '7px 10px', fontWeight: 600, color: 'var(--color-text-secondary)' }}>{t}</td>
                <td style={{ padding: '7px 10px', fontWeight: 700, color: a.startsWith('+') ? 'var(--color-success)' : 'var(--color-danger)' }}>{a}</td>
                <td style={{ padding: '7px 10px' }}>
                  <span style={{
                    ...
                    color: s === 'Completed' ? 'var(--color-success)' : 'var(--color-danger)',
                  }}>{s}</span>
                </td>
              </tr>
            ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        {[
          { title: 'Main Wallet', value: '$1,250.00', accent: 'var(--color-primary)' },
          { title: 'Rebate', value: '$45.80', accent: 'var(--color-success)' },
        ].map(c => (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {[
        ...
        { type: 'info', bg: 'var(--color-accent-pale)', border: 'var(--color-accent)', color: 'var(--color-secondary)', text: 'Your VIP status will be reviewed on Monday.' },
      ].map(a => (
...
function InputShowcase() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {/* Form fields */}
      <div>
        <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 4 }}>Username</label>
      ...
      <div>
        <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--color-text-secondary)', display: 'block', marginBottom: 4 }}>Password</label>

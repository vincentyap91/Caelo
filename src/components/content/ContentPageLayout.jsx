    ...
    return (
        ...
        >
            {items.map((item) => (
                <span
                    key={item}
                    className={`inline-flex min-w-0 shrink-0 items-center justify-center rounded-full border text-center text-xs font-bold uppercase leading-tight tracking-wide text-[var(--color-secondary)] ${
                        heroMobileOverlay
                            ? 'min-h-[2.5rem] w-full border-[var(--color-accent-glow)] bg-[var(--color-tertiery)]/92 px-2 py-2 shadow-[var(--shadow-card-soft)] backdrop-blur-sm'
                            : 'border-[var(--color-accent-glow)] bg-[var(--color-tertiery)] px-3.5 py-2 shadow-[var(--shadow-subtle)] sm:px-4'
                    }`}
                >
                    {item}
                </span>
    ...
    return (
        <section className={`surface-card rounded-[24px] p-5 md:p-6 ${className}`.trim()}>
            <div className="flex items-start gap-4">
                {Icon ? (
                    <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-[var(--color-accent-glow)] bg-[var(--color-accent-pale)] text-[var(--color-secondary)] shadow-[var(--shadow-subtle)]">
                    ...
                    >
                        {heroVisualSrc ? (
                            <div className="w-full max-w-[28rem] max-md:max-w-[58%] max-md:min-w-0 max-md:pb-32 max-md:pr-0 pr-10 sm:max-w-[31rem] sm:pr-16 md:block md:max-w-[57%] md:pb-0 md:pr-8 lg:max-w-[54%]">
                                {eyebrow ? (
                                    <p className="text-xs font-bold uppercase tracking-code text-[var(--color-secondary)]">
                                        {eyebrow}
                                    </p>
                                ) : null}
                            ...
                            <div className="max-w-4xl">
                                {eyebrow ? (
                                    <p className="text-xs font-bold uppercase tracking-wide text-[var(--color-secondary)]">{eyebrow}</p>
                                ) : null}

/* use_figma — Bind Referral - My Rewards (referral.jsx MyRewardsContent, VARIABLE-RULES.en.md §13.11) */
...
async function bindHistoryTable(node) {
  ...
  for (const row of node.findAll((n) => n.name === "Row")) {
    ...
    for (const badge of row.findAll((n) => n.name === "Data" && n.findOne((t) => t.type === "TEXT"))) {
      const statusText = badge.findOne((t) => t.type === "TEXT")?.characters || "";
      if (/unclaimed/i.test(statusText)) {
        badge.name = "referral-status-badge--unclaimed";
        await bindSolidFillPath(badge, "color/accent/pale");
      } else if (/claimed/i.test(statusText)) {
        badge.name = "referral-status-badge--claimed";
        await bindSolidFillPath(badge, "color/surface/subtle");
      }
    }
...
const contentPanel = await figma.getNodeByIdAsync("131:6084");
if (contentPanel) {
  contentPanel.name = "soft-blue-panel";
  await bindStrokePath(contentPanel, "color/accent/glow", 1);
}

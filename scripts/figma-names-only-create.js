...
const primNames = JSON.parse(figma.root.getSharedPluginData("caelo", "primNames") || "[]");
const semNames = JSON.parse(figma.root.getSharedPluginData("caelo", "semNames") || "[]");
if (!primNames.length) throw new Error("Run figma-names-only-store.js first");
const primResult = await addNames(primCol, [primCol.modes[0].modeId], primNames, false);
const semResult = await addNames(semCol, semCol.modes.map((m) => m.modeId), semNames, true);
